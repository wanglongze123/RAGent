...
### 问题
master_agent 每轮都用 Doubao 做 JSON 模式意图分类，单次 5-15s，导致用户发完消息后要等十几秒才看到第一个事件，远超项目要求"首 Token < 1s"。

### 解决
在 `_classify_intent` 前加纯规则快速通道 `_quick_classify(message, current_state)`：
- 强信号词（加购/删除/对比/下单/推荐 …）→ 直接返回 intent，跳过 LLM
- `cart_management` 状态下未识别的消息默认 `cart_add`（接续 SKU 回答场景）
- `checkout` 状态下任何消息都是 checkout
- 真正模糊的消息才走 LLM 兜底

### 实测对比

| 步骤 | 改造前首 Token | 改造后首 Token |
|---|---|---|
| 搜索"推荐一款洁面" | 5-15s | **19ms** |
| 加购"加购第一个" | 5-15s | **7ms** |
| SKU 回答"120g 标准装" | ~13s | **9ms** |
| 下单 6 步（确认/姓名/手机/地址/提交） | 5-15s × 6 | **3-7ms × 6** |

13/13 单元用例通过；端到端搜索→加购→选规格→下单完整链路（9 步）首 Token 全部 ≤ 19ms。

### 取舍

模糊场景（"为什么这个比那个贵"、"随便看看"）仍走 LLM —— 这是设计权衡：
规则越激进越快但越容易误判，保守拒绝让 LLM 兜底是更可控的策略。
后续可以把 `_quick_classify` 命中率作为 metric 暴露出来评估覆盖度。
